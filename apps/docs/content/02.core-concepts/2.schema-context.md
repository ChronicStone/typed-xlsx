...
- `style`
- `hyperlink`
- lazy table titles, headers, group headers, summary labels, totals-row labels, validation messages, and hyperlink tooltips
- badge variant `label` callbacks
- formula callbacks
...
const schema = createExcelSchema<Deal, SchemaContext>()
  ...
  .column("amount", {
    header: ({ ctx }) => `${ctx.currencySymbol} amount`,
    accessor: "amount",
...
Lazy text is resolved when a table receives its `context`, so the same schema can be reused for different locales:

```ts twoslash
import { createExcelSchema, createWorkbook } from "typed-xlsx";

type Row = { status: string };
type Context = { t(key: string): string };

const schema = createExcelSchema<Row, Context>()
  .column("status", {
    accessor: "status",
    header: ({ ctx }) => ctx.t("columns.status"),
    summary: (summary) => [summary.label(({ ctx }) => ctx.t("totals.label"))],
  })
  .build();
...
createWorkbook()
  ...
  .table("statuses", {
    ...
    rows: [],
    schema,
    title: ({ ctx }) => ctx.t("reports.statuses.title"),
  });
```
