...
description: Choose between createWorkbook and createWorkbookStream based on memory shape, API flow, and export scale.
---

typed-xlsx ships two workbook builders. Both support the same schema kinds, formula features, summaries, styling, and sheet options. The difference is in how row data flows through memory and when output is finalized.

```mermaid
flowchart TD
  S["Schema"] --> B["createWorkbook()<br/><small>all rows in memory</small>"]
  S --> ST["createWorkbookStream()<br/><small>commit batches to spool</small>"]
  B --> O1["toBuffer() / writeToFile()"]
  ST --> O2["writeToFile() / pipeToNode()"]
```

For the concrete APIs, see [Building Workbooks](/workbook/building-workbooks), [Streaming Intro](/streaming/streaming-intro), and [Memory Tuning](/streaming/memory-tuning).
