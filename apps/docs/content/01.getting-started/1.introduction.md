...
| Renderer columns          | Sparklines, badges, checkboxes, hyperlinks, embedded or URL-backed images — declared per column              |
...
```ts twoslash
import { createWorkbook, createWorkbookStream } from "typed-xlsx";

declare const rows: Array<Record<string, unknown>>;
declare const schema: import("typed-xlsx").SchemaDefinition<Record<string, unknown>>;
...
| ---------------------------------------------------- | ---------------------------------------------------------- |
| Complex reports from TypeScript data models          | `typed-xlsx` — typed schemas, formulas, summaries, styling |
...
| Parsing, rewriting, or broader spreadsheet workflows | A lower-level spreadsheet toolkit                          |
