...
| Merged header cells | Yes         | Structural groups stay flat |
...
Structural groups (`.group()`) and runtime-generated scopes (`.dynamic()`) are supported in excel-table mode with flat headers only. Runtime-generated columns are useful for grouped selection and scope formulas like `fx.sum(refs.dynamic("id"))`. Excel table mode still renders a flat single header row — there is no merged/group header band.
...
AutoFilter adds native Excel filter and sort dropdowns to the column headers.
...
AutoFilter is silently disabled in report-mode tables that use sub-row expansion because Excel cannot filter over merged cells.
