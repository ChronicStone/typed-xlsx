...
description: How createWorkbookStream() works, when to use it, and how to commit row batches.
---
...
For the exact memory trade-offs of `tempStorage`, `tempDirectory`, and string strategy, see [Memory Tuning](/streaming/memory-tuning).
...
for (const batch of [[{ date: "2024-01-01", region: "North", revenue: 4200 }]]) {
...
For output methods after committing batches, see [Output Targets](/streaming/output-targets).
