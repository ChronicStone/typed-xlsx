...
`createWorkbookStream()` is new in v1. There is no equivalent in v0 — the old `streaming: true` flag on `.build()` still materialized the full workbook before writing. See the [Streaming](/streaming/overview) section for full documentation.
