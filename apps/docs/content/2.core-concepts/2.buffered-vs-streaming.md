...
typed-xlsx ships two workbook builders. Both support the same schema kinds (report and excel-table), the same formula columns, the same groups and dynamic columns, and the same output targets. The difference is in how row data flows through memory.
