---
title: Patterns
description: Practical guidance for large exports — builder choice, batch sizing, auto-width, sub-row expansion, and multi-sheet.
---
...
buffered.sheet("Data").table("data", { rows: await fetchAll(), schema });

// Correct approach for 500k+ rows
const stream = createWorkbookStream({ tempStorage: "file" });
const table = await stream.sheet("Data").table("data", { schema });
...
const table = await workbook.sheet("Data").table("rows", { schema });
...
createExcelSchema<Order>()
  ...
  .column("sku", {
    accessor: "lines",
    transform: ({ value }) => value.map((line) => line.sku),
  })
  .column("qty", {
    accessor: "lines",
    transform: ({ value }) => value.map((line) => line.qty),
  })
  .build();
```
...
For environments with tight memory or restricted filesystem access, choose settings based on the runtime:

- no writable filesystem: `tempStorage: "memory"` may be necessary, but heap will grow with serialized XML
- writable filesystem available: prefer `tempStorage: "file"` to keep heap much lower
...
If disk access is unavailable, switch `tempStorage` to `"memory"` and test with production-sized exports.
...
const q1 = await workbook.sheet("Q1").table("q1", { schema });
const q2 = await workbook.sheet("Q2").table("q2", { schema });
const q3 = await workbook.sheet("Q3").table("q3", { schema });
const q4 = await workbook.sheet("Q4").table("q4", { schema });
