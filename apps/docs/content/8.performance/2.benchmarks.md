---
title: Benchmarks
description: Benchmark notes for the current stream benchmark tooling and how to run it.
---

The repository ships dedicated stream benchmark tooling under `packages/benchmark/`. Benchmark generation is manual for now. When you run it, artifacts are written under `packages/benchmark/artifacts/reports/stream` and mirrored into the docs app at build time if present.

## Benchmark setup

The benchmark generates synthetic data with a representative schema and records throughput, output size, and memory behavior for stream exports. Treat the numbers below as directional until regenerated for your machine and current branch.
...
The current benchmark case accepts `--rows` and `--batchSize`. Reports are written under `packages/benchmark/artifacts/reports/stream`.

Benchmarks are intentionally not run as part of the docs dev startup flow.
