...
```mermaid
flowchart LR
  A["accessor<br/><small>path or callback</small>"]
  T["transform<br/><small>optional</small>"]
  D["defaultValue<br/><small>if null/undefined</small>"]
  C["Cell value"]
...
| `condition` | `({ ctx }) => boolean`     | Structurally include or omit the column based on schema context. |
...
`condition` is available on every column definition, including accessor columns and formula columns.

Use it when a column should exist only for some schema-context configurations. It is structural only, so it should depend on `ctx`, not on row values.
...
const schema = createExcelSchema<Deal, { showFinance: boolean }>()
  ...
  .column("marginPct", {
    header: "Margin %",
    formula: ({ refs, fx }) => fx.safeDiv(refs.column("margin"), refs.column("amount")),
    condition: ({ ctx }) => ctx.showFinance,
    style: { numFmt: "0.0%" },
  })
  .build();
...
For the full schema-context model behind `condition`, see [Schema Context](/core-concepts/schema-context).
...
Columns support native Excel data validation via the `validation` field. See [Data Validation](/columns/data-validation) for the full walkthrough.

## Hyperlinks

Columns support native Excel hyperlinks via the `hyperlink` field. See [Hyperlinks](/columns/hyperlinks) for the full walkthrough.

Columns can also render native Excel sparklines via the `sparkline` field. See [Sparklines](/columns/sparklines) for source typing, theming, and supported options.

Columns can render embedded PNG/JPEG thumbnails or URL-backed Excel `IMAGE()` formulas via `type: "image"`. See [Images](/columns/images) for media sources, portability tradeoffs, and file-size constraints.

Columns can render status badges and native checkbox cells via `type: "badge"` and `type: "checkbox"`. See [Badges and Checkboxes](/columns/badges-and-checkboxes) for the renderer API.
