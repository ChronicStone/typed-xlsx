...
- `hyperlink: ({ row, rowIndex, subRowIndex, ctx }) => string | { target, tooltip?, style? } | null | undefined`

Returning `null` or `undefined` means that specific cell has no hyperlink.

`tooltip` can be a string or a lazy callback, so shared schemas can resolve localized tooltip text when rows are planned.
