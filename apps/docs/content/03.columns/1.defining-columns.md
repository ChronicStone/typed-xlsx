...
The simplest accessor is a dot-path string. xlsmith infers all valid paths from `T` — you get autocomplete and a type error if the path doesn't exist:
...
If an accessor or transform returns an array, xlsmith expands the logical row into multiple physical rows and merges single-value cells automatically. This is a **report-mode only** feature.
