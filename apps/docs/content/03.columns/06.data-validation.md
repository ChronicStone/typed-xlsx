...
const schema = createExcelSchema<{ status: string }, Context>()
  .column("status", {
    header: ({ ctx }) => ctx.t("columns.status"),
    accessor: "status",
    validation: (validation) =>
      validation.list(["draft", "active", "archived"]).error({
        title: ({ ctx }) => ctx.t("validation.status.title"),
        message: ({ ctx }) => ctx.t("validation.status.message"),
      }),
  })
  .build();
```

The callbacks receive the table's schema context. The library does not know about any i18n framework directly, so applications can pass their own `t` function.
