...
description: Build XLSX files with createWorkbook() — sheets, tables, layout, and final output methods.
---
...
Use `defaults` on `.table()` to define table-wide styling defaults for headers, summaries, body cell states, and body row height.
...
`rowHeight` is a minimum body-row height in points. It is useful for visual cells such as sparklines while still allowing taller wrapped rows to expand.
...
Choose exactly one finalization path per workbook instance.
...
For stream-specific output shapes like `toNodeReadable()` and `pipeToNode()`, see [Output Targets](/streaming/output-targets).
