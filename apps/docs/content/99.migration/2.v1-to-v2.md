...
.column("net", {
  formula: ({ row, fx }) =>
    fx.round(
      row.ref("contractValue").mul(fx.literal(1).sub(row.ref("discountPct"))),
      2,
    ),
  style: { numFmt: '"$"#,##0.00' },
})
...
| API                                                                            | Purpose                                                                                    |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| `row.ref("columnId")`                                                          | Reference the current row's cell for a preceding column                                    |
| `row.series("columnId")`                                                       | Reference the full expanded range of a column within the current logical row (report mode) |
| `row.group("groupId")`                                                         | Aggregate all columns in a dynamic group                                                   |
...
.column("attainment", {
  ...
  conditionalStyle: (conditional) =>
    conditional
      ...
      .when(({ row, fx }) =>
        fx.and(row.ref("attainment").gte(1), row.ref("status").eq("won")),
        {
...
.column("score", {
  accessor: "score",
  validation: (v) =>
    v.custom(({ row }) => row.ref("score").gte(row.ref("amount"))),
})
```
...
v1 groups were limited to dynamic column generation. v2 makes groups first-class in the formula scope.
...
createExcelSchema<Row>()
  .group("regions", (group, regions: string[]) => {
    for (const region of regions)
      group.column(region, { accessor: (row) => row.byRegion[region] ?? 0 });
  })
  .build();

// v2 — formulas can reference and aggregate groups
createExcelSchema<Row>({ mode: "excel-table" })
  .group("regions", (group, regions: string[]) => {
    for (const region of regions)
      group.column(region, {
        header: region,
...
| Group formulas       | Groups generate columns only          | `row.group("id").sum()` / `.average()` / etc.                   |
| Formula row model    | Not available                         | `row.ref()` / `row.series()` / `expansion`                      |
