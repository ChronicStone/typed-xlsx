...
description: Complete reference for the formula DSL — operands, conditions, functions, group aggregates, summary formulas, and scope rules.
---

This page is the exhaustive reference for the formula DSL. For a guided introduction with worked examples, see [Formula Columns](/formulas/formula-columns).
...
Embeds a constant string, number, or boolean into the formula. Returns a `FormulaOperand`.
...
References a group of columns declared with `.group()`. Returns a `FormulaGroupContext`. See [group aggregates](#group-aggregates) below.

---

## `FormulaOperand` — arithmetic and comparison

A `FormulaOperand` is returned by `row.ref()`, `row.literal()`, and most `fx.*` functions. It exposes arithmetic operators, comparison operators, and unary functions chainably.
...
createExcelSchema<{ bid: number; ask: number; mid: number }>()
  ...
  .column("spread", {
    formula: ({ row, fx }) => fx.max(row.ref("ask").sub(row.ref("bid")), row.literal(0)),
  })
  .build();
```
...
Scope rules (predecessor ordering, group scope, runtime errors) and how mode affects the emitted formula string (A1 vs structured references) are covered in [Scope and Modes](/formulas/scope-and-modes).
