...
Report mode can expand one logical row into multiple physical worksheet rows. That row model is explained in depth in [Row Model](/formulas/row-model), including when to use `row.ref(...)`, `row.series(...)`, `column.cells()`, and `column.rows()`.
...
flowchart TD
  ...
  D --> E[group aggregate exposed outside via row.group]
