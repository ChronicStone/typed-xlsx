...
`row.literal(value)` embeds a constant into the formula. Useful for thresholds, rates, or magic numbers that should live in the Excel formula rather than the JavaScript layer:
...
const schema = createExcelSchema<{ amount: number }>()
  ...
  .column("amountWithVat", {
    ...
    formula: ({ row }) => row.ref("amount").mul(row.literal(1.2)),
    style: { numFmt: "$#,##0.00" },
  })
  .build();
```
