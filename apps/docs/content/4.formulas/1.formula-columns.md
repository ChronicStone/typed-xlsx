...
Operands returned by `refs.column()` and `fx.*` functions all expose `.add()`, `.sub()`, `.mul()`, `.div()`. Primitive literals can be passed directly:
...
const schema = createExcelSchema<{
  ...
  .column("marginPct", {
    ...
    formula: ({ row, refs }) =>
      refs.column("revenue").sub(refs.column("cost")).div(refs.column("revenue")),
    style: { numFmt: "0.0%" },
  })
  .column("taxOwed", {
    header: "Tax Owed",
...
const schema = createExcelSchema<{
  ...
  .column("utilization", {
    ...
    formula: ({ row, refs, fx }) =>
      fx.if(
        refs.column("purchasedSeats").gt(0),
        fx.round(refs.column("activatedSeats").div(refs.column("purchasedSeats")), 4),
        0,
      ),
    style: { numFmt: "0.0%" },
  })
