...
type RenderedExample = ExampleDefinition;

const activeId = ref<ExampleId>("report");
const iframeFailed = ref(false);
const runtimeConfig = useRuntimeConfig();
const githubExamplesRoot =
  "https://github.com/ChronicStone/typed-xlsx/tree/main/packages/examples/src";

const financialReportArtifact = findExampleArtifact("financial-report");
const kitchenSinkArtifact = findExampleArtifact("kitchen-sink");

const rendered = computed<RenderedExample[]>(() =>
  EXAMPLES.map((example) => {
    if (example.id === "report" && financialReportArtifact) {
      return {
        ...example,
        workbookUrl: `/generated/examples/reports/${financialReportArtifact.reportPath}`,
        sourceUrl: `${githubExamplesRoot}/financial-report`,
        code: financialReportArtifact.sourceFiles["workbook.ts"],
      };
    }

    if (example.id === "formulas" && kitchenSinkArtifact) {
      return {
        ...example,
        sourceUrl: `${githubExamplesRoot}/kitchen-sink/schema.ts`,
        code: kitchenSinkArtifact.sourceFiles["schema.ts"],
      };
    }

    if ((example.id === "table" || example.id === "composition") && kitchenSinkArtifact) {
      return {
        ...example,
        workbookUrl: `/generated/examples/reports/${kitchenSinkArtifact.reportPath}`,
        sourceUrl: `${githubExamplesRoot}/kitchen-sink/buffered.ts`,
        code: kitchenSinkArtifact.sourceFiles["buffered.ts"],
      };
    }

    if (example.id === "streaming" && financialReportArtifact) {
      return {
        ...example,
        sourceUrl: `${githubExamplesRoot}/financial-report/stream.ts`,
        code: financialReportArtifact.sourceFiles["stream.ts"],
      };
    }
...
<template>
  ...
  >
    ...
    <div class="showcase-split grid grid-cols-1 lg:grid-cols-2">
      ...
      >
        <MdcCodeBlock :code="current.code" lang="ts" class="showcase-code" />
      </div>
