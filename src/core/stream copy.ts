...
> {
  ...
  /**
   * Process a single chunk of data without using cell merging
   */
  private async processChunk(chunk: T[]): Promise<void> {
    ...
    for (let rowIdx = 0; rowIdx < chunk.length; rowIdx++) {
      ...
      const processedValues = flatColumns.map((column, colIdx) => {
      ...
      for (let subRowIdx = 0; subRowIdx < maxArrayLength; subRowIdx++) {
        const rowValues = processedValues.map((processed, colIdx) => {
          ...
          // For non-array values, only show in first row and use empty string for others
            return subRowIdx === 0 ? processed.value : null
          }
        })
        ...
        // Skip empty cells (null values)
          ...
          // For array values, get the style for the specific subrow
          ...
          const cellWidth = calculateColumnWidth(rowValues[colIdx])
  ...
  public async commit(): Promise<void> {
    ...
    try {
      ...
      for (const column in this.columnWidths)
        console.log('Final column width', column + 1, this.worksheet.getColumn(+column + 1).width)
