...
const schema = createSchema<SaleRow>()
...
const orderSchema = createSchema<{ id: string; total: number }>()
  .column("id", { accessor: "id" })
  .column("total", { accessor: "total", style: { numFmt: "#,##0.00" } })
  .build();

const logSchema = createSchema<{ ts: string; message: string }>()
...
const schema = createSchema<{ label: string; value: number }>()
...
const schema = createSchema<{ id: string; name: string; internalNote: string }>()
  .column("id", { accessor: "id" })
