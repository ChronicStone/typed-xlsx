...
In practice, this is the schema object you pass into `.table("table-id", { schema, ... })`.
...
The built schema also preserves formula scope metadata:

- `row.ref(...)` stays limited to previously declared columns in scope
- `row.group(...)` is limited to previously declared group ids
