...
## Formula scope with groups

Formulas now follow a simple scope model:

- `row.ref("...")` targets a single previously declared column in scope
- inside `group(...)`, formulas can reference previous outer columns and previous local group columns
- later formulas can target a whole previously declared group with `row.group("groupId")`
...
Supported group aggregations are:
...
- future column refs
- future group refs
- direct typed refs to runtime-generated child ids from outside the group
