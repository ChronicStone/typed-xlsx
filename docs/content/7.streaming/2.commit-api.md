...
const table = await sheet.table("sales", { schema });
...
const ordersTable = await ordersSheet.table("orders", { schema: orderSchema });
...
const logTable = await logSheet.table("log", { schema: logSchema });
...
const tableA = await sheet.table("by-region", { schema });
const tableB = await sheet.table("by-category", { schema });
...
const table = await workbook
  .sheet("Export")
  .table("data", { schema, select: { exclude: ["internalNote"] } });
...
The right number depends on your row's shape (wide vs narrow), string cardinality, and available memory. For pipelines backed by a database cursor or paginated API, use whatever page size the source produces.
