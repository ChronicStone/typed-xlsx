...
```ts twoslash
// @noErrors
import { createSchema, createWorkbook, createWorkbookStream } from "@chronicstone/typed-xlsx";
...
const schema = createSchema<{ id: string }>().column("id", { accessor: "id" }).build();
...
createSchema<{ id: string; description: string; amount: number }>()
...
createSchema<Order>()
  ...
  .column("sku", { accessor: "lines", transform: (lines) => lines.map((l) => l.sku) })
  .column("qty", { accessor: "lines", transform: (lines) => lines.map((l) => l.qty) })
  .build();
```
