...
The repository currently ships benchmark tooling for stream-mode exports. The benchmark scripts live under `packages/typed-xlsx/scripts/` and write reports under `.reports/benchmarks`.
...
The current benchmark case accepts `--rows` and `--batchSize`. Reports are written under `.reports/benchmarks`.
