...
A common pattern is one canonical report schema used for multiple exports — full data, filtered columns, different audience. The builder system (`createExcelSchema()`) produces a plain definition object that carries no state, so it can be passed to any number of tables across multiple workbooks without side effects. Native Excel table schemas use the same entry point with `createExcelSchema({ mode: "excel-table" })`.
