...
export const financialReportSchema = ExcelSchemaBuilder.create<FinancialReport>()
  ...
  .column("Profit Margin", {
    ...
    cellStyle: (data, _, valueIndex) => ({
      font: {
        color: { rgb: data.departments[valueIndex].profitMargin >= 0 ? "007500" : "FF0000" },
      },
    }),
  })
  .column("totalRevenue", {
    ...
    summary: [
      {
        ...
        cellStyle: (data) => ({
          font: {
            color: {
              rgb: data.reduce((acc, item) => acc + item.totalProfit, 0) >= 0 ? "007500" : "FF0000",
            },
          },
        }),
      },
    ...
    summary: [
      {
        ...
        cellStyle: (data) => ({
          font: {
            color: {
              rgb:
                data.reduce((acc, item) => acc + item.averageProfitMargin, 0) / data.length >= 0
                  ? "007500"
                  : "FF0000",
            },
          },
